...
STEP7_ROWS = [
    ...
    },
    {
        "title_key": "p3_15",
        "title_text": "3.2.P.3 제조\n15. 완제의약품의 제조공정 변경",
        "output_condition_all_met": "(\n    step6_selections.get(\"p3_15_sub_15b\") == \"변경 있음\" and step6_selections.get(\"p3_15_req_1\") == \"충족\" and step6_selections.get(\"p3_15_req_2\") == \"충족\" and step6_selections.get(\"p3_15_req_3\") == \"충족\" and step6_selections.get(\"p3_15_req_5\") == \"충족\" and step6_selections.get(\"p3_15_req_6\") == \"충족\" and step6_selections.get(\"p3_15_req_7\") == \"충족\" and step6_selections.get(\"p3_15_req_8\") == \"충족\" and step6_selections.get(\"p3_15_req_9\") == \"충족\" and step6_selections.get(\"p3_15_req_4\") == \"미충족\"\n)",
        "output_1_tag": "IR",
        "output_1_text": "보고유형은 다음과 같습니다.\n\nIR, 시판전보고\n「의약품의 품목허가‧신고‧심사 규정」 제3조의2제4항 단서조항에 따른 시판전 보고(Immediate Report, IR) 수준의 변경사항입니다.",
...
if st.session_state.step == 7:
    ...
    if visible_results:
        tag, output1, output2 = visible_results[0]
        st.session_state.step7_results[current_key] = {
...
TEMPLATE_PATH = Path(__file__).parent / "제조방법변경 신청양식_empty_.docx"

def _style(cell, size=11):
    for p in cell.paragraphs:
        for r in p.runs:
            r.font.size = Pt(size)
        p.paragraph_format.line_spacing = 1.4

def create_application_docx(tk, res, reqs, sel):
    doc = Document(TEMPLATE_PATH)
    h0 = doc.add_heading(
        "「의약품 허가 후 제조방법 변경관리 가이드라인(민원인 안내서)」[붙임] 신청양식 예시",
        level=0,
    )
    h0.runs[0].font.size = Pt(12)
    h0.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER

    # 1. 신청인
    doc.add_heading("1. 신청인", level=1).runs[0].font.size = Pt(11)
    t1 = doc.add_table(rows=3, cols=2, style="Table Grid")
    for i, txt in enumerate(("성명", "제조소(영업소) 명칭", "변경신청 제품명")):
        t1.cell(i, 0).text = txt
        _style(t1.cell(i, 0)); _style(t1.cell(i, 1))

    # 2. 변경유형
    doc.add_heading("2. 변경유형", level=1).runs[0].font.size = Pt(11)
    t2 = doc.add_table(rows=1, cols=1, style="Table Grid")
    t2.cell(0, 0).text = res["title_text"]; _style(t2.cell(0, 0))

    # 3. 신청유형
    doc.add_heading("3. 신청유형", level=1).runs[0].font.size = Pt(11)
    t3 = doc.add_table(rows=2, cols=2, style="Table Grid")
    t3.cell(0, 0).text, t3.cell(0, 1).text = "분류", res["output_1_tag"]
    t3.cell(1, 0).text = "설명"
    for ln in res["output_1_text"].splitlines():
        run = t3.cell(1, 1).paragraphs[0].add_run(ln); run.add_break()
    for row in t3.rows:
        for cell in row.cells: _style(cell)

    # 4. 충족조건
    doc.add_heading("4. 충족조건", level=1).runs[0].font.size = Pt(11)
    items = list(reqs.items())
    n_rows = max(5, min(15, len(items)))
    t4 = doc.add_table(rows=n_rows, cols=2, style="Table Grid")
    for i in range(n_rows):
        if i < len(items):
            rk, txt = items[i]
            stt = sel.get(f"{tk}_req_{rk}", "")
            sym = "○" if stt == "충족" else "×" if stt == "미충족" else ""
            t4.cell(i, 0).text, t4.cell(i, 1).text = txt, sym
        _style(t4.cell(i, 0)); _style(t4.cell(i, 1))

    # 5. 필요서류
    doc.add_heading("5. 필요서류", level=1).runs[0].font.size = Pt(11)
    docs = [l for l in res["output_2_text"].splitlines() if l.strip()]
    n_docs = max(5, min(15, len(docs)))
    t5 = doc.add_table(rows=n_docs, cols=1, style="Table Grid")
    for i in range(n_docs):
        t5.cell(i, 0).text = docs[i] if i < len(docs) else ""
        _style(t5.cell(i, 0))
...
if st.session_state.step == 8:
    flat_keys = [(tk, i) for tk, lst in st.session_state.step7_results.items() for i in range(len(lst))]
    if "step8_page" not in st.session_state:
        st.session_state.step8_page = 0
    page = st.session_state.step8_page
    tk, idx = flat_keys[page]
    res = st.session_state.step7_results[tk][idx]
    reqs = st.session_state.step6_items[tk]["requirements"]
    sel = st.session_state.step6_selections
    doc_path = create_application_docx(tk, res, reqs, sel)

    c1, c2, c3 = st.columns([1,1,1])
    with open(doc_path, "rb") as f:
        c1.download_button("📄 파일 다운로드", f, file_name=f"신청서_{tk}_{idx+1}.docx")
    c2.markdown(f"<h5 style='text-align:center;font-weight:bold;'>{page+1} / {len(flat_keys)}</h5>", unsafe_allow_html=True)
    if c3.button("🖨 인쇄하기"):
        st.markdown("<script>window.print();</script>", unsafe_allow_html=True)

    st.markdown("<h4 style='text-align:center;'>「의약품 허가 후 제조방법 변경관리 가이드라인(민원인 안내서)」[붙임] 신청양식 예시</h4>", unsafe_allow_html=True)

    def row2(a,b=""): return f"<tr><td>{a}</td><td>{b}</td></tr>"
    tbl1 = row2("성명") + row2("제조소(영업소) 명칭") + row2("변경신청 제품명")
    cond_html = "".join(
        f"<tr><td>{txt}</td><td style='text-align:center'>{'○' if sel.get(f'{tk}_req_{rk}')=='충족' else '×' if sel.get(f'{tk}_req_{rk}')=='미충족' else ''}</td></tr>"
        for rk, txt in reqs.items()
    )
    doc_html = "".join(f"<tr><td>{l}</td></tr>" for l in res["output_2_text"].splitlines() if l.strip())
    st.markdown(f"""
    <style>table,td,th{{border:1px solid #000;border-collapse:collapse;padding:6px;}}td,th{{font-size:14px;line-height:1.4;}}</style>
    <h5>1. 신청인</h5><table>{tbl1}</table><br>
    <h5>2. 변경유형</h5><table><tr><td>{res['title_text']}</td></tr></table><br>
    <h5>3. 신청유형</h5><table>{row2('분류', res['output_1_tag'])}<tr><td colspan='2'>{res['output_1_text'].replace('
','<br>')}</td></tr></table><br>
    <h5>4. 충족조건</h5><table><tr><th>충족조건</th><th>조건 충족 여부</th></tr>{cond_html}</table><br>
    <h5>5. 필요서류</h5><table><tr><th>서류</th></tr>{doc_html}</table>
    """, unsafe_allow_html=True)
    ...
    b_next.button("다음 ➡", disabled=page == len(flat_keys)-1,
                  on_click=lambda: st.session_state.update(step8_page=page+1) or st.experimental_rerun()): 신청양식 PDF 생성 =====
